# JavaScript Interview Prep: 53 Tricky Questions on `var`, `let`, and `const` (Scope & Hoisting)

This document contains a comprehensive list of 53 tricky JavaScript interview questions gathered from real-world tech interviews, focused on the differences, scope, and hoisting behaviors of `var`, `let`, and `const`, complete with outputs and explanations.
...
## 51. ES6 Module Scope and the Global Object

**Question:** In an HTML file with `<script type="module">`, does `var` behave like it does in a standard script?
...
**Explanation:**
In standard script tags, top-level `var` declarations add properties to the global object (`window` in browsers). However, ES6 modules (`<script type="module">`) each have their own top-level module scope. Variables declared with `var`, `let`, or `const` at the top level of a module are scoped to that module and do not become properties of the global object.

</details>
---

## 52. Garbage Collection: `let` in Loops vs. IIFE Closures

**Question:** How does the modern `let` loop pattern differ from the old IIFE pattern regarding garbage collection and memory retention over time?
...
**Explanation:**
Both achieve the same result initially, but their internal mechanisms differ.
The modern `for(let i...)` loop implicitly creates a fresh lexical environment (a new hidden block scope) for *each iteration* that captures the current value of `i`. These environments are held in memory as long as the `setTimeout` closure holds a reference to them. Once the callbacks run and are disposed of, the associated environments can be garbage collected individually.

The older IIFE pattern manually forces a new function execution context, copying the primitive value `j` as a new local variable `index`. The difference in memory footprint is generally negligible for primitives, but if the loop contained large objects in its closure, understanding how those lexical environments are retained by the closures (and when they can be garbage collected) is vital. The `let` loop is engine-optimized and avoids the slight overhead of repeated function invocation (the IIFEs).

</details>
---

## 53. Named Function Expressions and Hoisting

**Question:** What happens if you try to call `b()` outside and inside this named function expression?
...
**Explanation:**
This is a Variable Declaration initialized with a *Named Function Expression*.
Only the variable `a` is hoisted (as `undefined`).
The identifier `b` is **not** added to the outer scope at all (which is why calling `b()` throws a `ReferenceError`). Instead, the name `b` is strictly bound locally *inside* the function body itself, allowing the function to reliably refer to itself recursively (hence `typeof b` logs `"function"` when invoked via `a()`).
